...
<form method="post" action="<?= $_SERVER['PHP_SELF']."?function=register"; ?>">
  ...
  <div>
	<input class="form-control" type="text" id="benutzername" name="benutzername" placeholder="Benutzername" required="required" />
  </div>
  ...
  <div>
	<input class="form-control" type="password" id="password" name="password" placeholder="Passwort" required="required" pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" title="Bitte geben sie ein Passwort mit mindestens 8 Zeichen, mindestens ein Grossbuchstabe, mindestens eine Zahl und mindestens ein Sonderzeichen"/>
  </div>
  <label for="passwort">Passwort Wiederholen</label>
  <div>
  <input class="form-control" type="password" id="confirmpassword" name="confirm_password" placeholder="Passwort Wiederholen" required="required"/>
  </div>
  <div>
