<?php
    ...
    foreach (getUserNames() as $user){
        if ($_GET['bid'] == $user[0]) {
            echo "<div><a href='index.php?function=blogs&bid=$user[0]' id='$user[0]' title='Blog von $user[1] auswählen'><u><h4>$user[1]</h4></u></a></div>";
        } else {
        echo "<div><a href='index.php?function=blogs&bid=$user[0]' id='$user[0]' title='Blog von $user[1] auswählen'><h4>$user[1]</h4></a></div>";
        }
    }
